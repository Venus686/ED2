...
int numHojas(const Arbin<T>& a) {
    int contador = 0; // Contador de hojas
    ...
    if (!a.esVacio()) {
        ...
        while (!c.empty()) {
            ...
            if (a.subIzq(ic).arbolVacio() && a.subDer(ic).arbolVacio()) {
                contador++;
            }
...
Arbin<T> simetrico(const Arbin<T>& a, const typename Arbin<T>::Iterador& r) {
    if (!r.arbolVacio()){ // si es un arbol vacio
        if(a.subIzq(r).arbolVacio() && a.subDer(r).arbolVacio()){ //si es una hoja
            return Arbin<T>(r.observar(),Arbin<T>(),Arbin<T>());//devuelvo el arbol con la raiz y la izquierda y la derecha vacia
        }else{
            return Arbin<T>(r.observar(),simetrico(a,a.subDer(r)),simetrico(a,a.subIzq(r)));
        }
    }else{
        return Arbin<T>();//devuelvo directamente el arbol
    }
}
/****************************************************************************/
//Ejercicio 3
template <typename T>
void recorridoZigzag(const Arbin<T>& a,const char z){
    return recorridoZigzag(a,a.getRaiz(), z);
}

template <typename T>
void recorridoZigzag(const Arbin<T>& a, const typename Arbin<T>::Iterador& r, const char z ) {
    if(!r.arbolVacio()){
        cout<< r.observar()<<" ";
        if(z=='I'){
...
// Si el árbol está vacío o es hoja esta compensado
// Si tiene hijos, si la altura de la derecha con la izquierda es >1 está descompensado y al revés igual no está compensado
// Si si lo está, miramos el hijo del hijo actual
template <typename T>
...
bool compensado(const Arbin<T>& a, const typename Arbin<T>::Iterador& r ) {
    if(r.arbolVacio() || (a.subIzq(r).arbolVacio() && a.subDer(r).arbolVacio())){
        ...
        cout<<"\n"<< a.subIzq(r).altura() <<" "<<a.subDer(r).altura();// mirar las alturas, para ver si te lo hace bn
        if(a.subDer(r).altura()-a.subIzq(r).altura()>=2 || a.subIzq(r).altura()-a.subDer(r).altura()<=-2){
            return false;
        }else{
...
int main(int argc, char *argv[])
{
    ...
    cout << "Num. hojas del arbol F: " << numHojas(F) << endl;
    ...
    recorridoZigzag(C, 'D');
    cout << endl << endl;
    cout << "Recorrido en zigzag I de F:\n";
    recorridoZigzag(F, 'I');
    cout << endl << endl;
